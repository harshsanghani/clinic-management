$(document).ready(function(){
    ...
    $(document).on('click', '.deleteToolData',function(e){
        ...
        if (confirm("Are you sure you wants to delete?")) {
            ...
            if (type != '' && id != '') {
                $.ajax({
                    url: base_url+'investigation/deleteTool',
    ...
    $(document).on('submit', '#modal-tool form', function(e) {
        ...
        if ($(this).valid()) {
            $.ajax({
                url: base_url+'investigation/commitTool',
                data: $(this).serialize(),
                type:"post",
                success: function(data){
                    var response =  $.parseJSON(data);
...
function openModal(selector) {
    ...
    if (type != "") {
        ...
        $.ajax({
            url: base_url+'investigation/openModal',
            data: form_data,
            type:"post",
            success: function(data){
                var response =  $.parseJSON(data);
                if (response.status == true) {
                    ...
                    setTimeout(function(){
                        $('#modal-tool input[type="text"]').focus();
                    }, 500);
                    formValidate();
                } else {
                    showMessage('danger',response.message);
...
function refreshToolData(type) {
    if (type != '') {
        ...
        $.ajax({
            url: base_url+'investigation/toolData',
            data : {tool_type : type},
            type:"post",
            success: function(data){
                var response =  $.parseJSON(data);
